...
def load_config(path="configs/default.yaml"):
    ...
    dl_kwargs = {
        ...
        "sequence_length": 1,      # 🔴 IMAGE = single frame
...
def main():
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc="Evaluating IMAGE"):
            frames = batch["frames"].to(device)   # (B, 1, C, H, W)
            labels = batch["label"].to(device)

            images = frames[:, 0]                 # (B, C, H, W)
            logits = model(images)
